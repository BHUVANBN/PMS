...
const UserDocuments = () => {
  ...
  const fetchDocs = async () => {
    ...
    try {
      setLoading(true);
      setError('');
      const res = await hrAPI.getHRGenericDocuments(userId);
  ...
  return (
    <Box>
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
        <div>
          <Typography variant="h4" fontWeight={700}>User Documents</Typography>
          <Typography variant="body2" color="text.secondary">Viewing documents sent to the selected user</Typography>
        </div>
        ...
        <Stack spacing={2}>
          {docs.length === 0 && (
            <Paper variant="outlined" sx={{ p: 2 }}>
              <Typography variant="body2" color="text.secondary">No documents available for this user.</Typography>
            </Paper>
          )}
